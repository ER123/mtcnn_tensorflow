...
def read_annotation(base_dir, label_path):
    ...
    with open(label_path, 'r') as f:
        labelfiles = f.readlines()
    print ("len(labelfiles):",len(labelfiles))

    for labelfile in labelfiles:
        # image path
        labelfile = labelfile.strip().split(' ')
        
        imagepath = base_dir + labelfile[0]
        ...
        # h, w, c = im.shape
        one_image_bboxes = []
        labelfile_ = list(map(int, labelfile[1:]))
        boxes = np.array(labelfile_, dtype=np.int32).reshape(-1, 14)
